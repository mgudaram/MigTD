...
pub fn create_logarea() -> Result<()> {
    ...
    #[cfg(not(test))]
    {
        ...
        for index in 0..num_vcpus {
            ...
            let mut provisional_buffer: alloc::boxed::Box<[u8; PAGE_SIZE]> = alloc::boxed::Box::new([0; PAGE_SIZE]);
    ...
    #[cfg(test)]
    {
        ...
        let bytes: &[u8] = logareabuffheader.as_bytes();
        databuffer[0..size_of::<LogAreaBufferHeader>()]
            .copy_from_slice(&bytes[0..bytes.len()]);
...
pub fn free_provisional_logarea() {
    let provisional_logs_enabled: bool = LOGGING_INFORMATION.provisional_logs_enabled.load(Ordering::SeqCst);
        ...
        #[cfg(not(test))]
        {
            for index in 0..num_vcpus {
                ...
                if provisional_logs_enabled {
                    ...
                    let provisional_buffer = unsafe { core::slice::from_raw_parts(provisional_buffer as *const u8, PAGE_SIZE) };
...
pub fn entrylog(msg: &Vec<u8>, loglevel: Level, request_id: u64) {
    ...
    if (logarea_initialized && log_level <= log_max_level) || (provisional_logs_enabled && log_level <= loglevel_to_u8(Level::Trace)) {
...
impl log::Log for VmmLoggerBackend {
    fn enabled(&self, metadata: &Metadata) -> bool {
        let mut log_max_level: LevelFilter;
...
mod test {
    ...
    fn test_create_logarea() {
        ...
        let logarea_created: bool = LOGGING_INFORMATION.logarea_created.load(Ordering::SeqCst);
        let provisional_logs_enabled: bool = LOGGING_INFORMATION.provisional_logs_enabled.load(Ordering::SeqCst);
        let logarea_initialized: bool = LOGGING_INFORMATION.logarea_initialized.load(Ordering::SeqCst);
    ...
    async fn test_provisional_and_entrylog_combined() {
        ...
        entrylog(&provisional_msg.to_string().into_bytes(), Level::Trace, u64::MAX);
        assert_eq!(LOGGING_INFORMATION.logentry_id.load(Ordering::SeqCst), 1);

        {
            let provisional_logareavector = PROVISIONAL_LOGAREAPTR.lock();
            let provisional_buffer_ptr = provisional_logareavector[0];
            let provisional_buffer =
                unsafe { core::slice::from_raw_parts(provisional_buffer_ptr as *const u8, PAGE_SIZE) };
        ...
        let result = enable_logarea(log_max_level, 0, &mut data).await;
        assert!(result.is_ok());
        assert!(!LOGGING_INFORMATION.provisional_logs_enabled.load(Ordering::SeqCst));
        assert!(LOGGING_INFORMATION.logarea_initialized.load(Ordering::SeqCst));
        ...
        let mut logareavector = LOGAREAPTR.lock();
        let data_buffer_ptr = logareavector[0];
        let data_buffer = unsafe { core::slice::from_raw_parts(data_buffer_ptr as *const u8, PAGE_SIZE) };
        ...
        let first_entry_bytes = &data_buffer[startoffset..startoffset + size_of::<LogEntryHeader>()];
