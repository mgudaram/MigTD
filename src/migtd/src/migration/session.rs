        ...
        poll_fn(|_cx| {
            ...
            if operation == DataStatusOperation::StartMigration as u8 {
                ...
                let expected_datalength =
                    size_of_val(&mig_request_id) + size_of::<MigtdStartMigrationInformation>();
                ...
                let mig_info = MigrationInformation {
                    ...
                    mig_request_id,
                    mig_info,
                    reportdata: [0; TD_REPORT_ADDITIONAL_DATA_SIZE],
                    loglevel: [0; 8],
                };
                ...
                let mut mig_request_id: u64 = 0;
                let mut logging: u64 = 0;
                let expected_datalength = size_of_val(&mig_request_id) + size_of_val(&logging);
                ...
                let slice = &data_buffer[12..12 + data_length as usize];
                mig_request_id = u64::from_le_bytes(slice[0..8].try_into().unwrap());
                logging = u64::from_le_bytes(slice[8..16].try_into().unwrap());
